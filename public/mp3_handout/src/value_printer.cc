...
static void embed_getelementptr(std::ostream &o, op_type type, operand op1,
...
void my_print_escaped_string(std::ostream &str, const char *s) {
  while (*s) {
    switch (*s) {
    case '\\':
      str << "\\5C";
 ...
 */
void ValuePrinter::init_constant(std::ostream &o, std::string name,
                                 const_value op) {
...
void ValuePrinter::init_constant(std::string name, const_value op) {
  check_ostream();
  init_constant(*stream, name, op);
}

void ValuePrinter::init_ext_constant(std::ostream &o, std::string name,
                                     op_type type) {
  o << "@" + name + " = " + "external constant " + type.get_name() + "\n";
}

void ValuePrinter::init_ext_constant(std::string name, op_type type) {
  check_ostream();
 ...
 * Note: Must terminate the function definition with a "}" or by using
 * end_define() after printing all the instructions in a function body.
 */
void ValuePrinter::define(std::ostream &o, op_type ret_type, std::string name,
                          std::vector<operand> args) {
  check_ostream(o);
  o << "define " + ret_type.get_name() + " @" + name + "(";
  for (unsigned i = 0; i < args.size(); ++i)
    o << args[i].get_typename() + " " + args[i].get_name() +
             (i + 1 < args.size() ? ", " : "");
  o << ") {\n";
}
void ValuePrinter::define(op_type ret_type, std::string name,
                          std::vector<operand> args) {
 ...
 */
void ValuePrinter::declare(std::ostream &o, op_type ret_type, std::string name,
...
void ValuePrinter::declare(op_type ret_type, std::string name,
                           std::vector<op_type> args) {
 ...
 * 	}
 */
void ValuePrinter::type_define(std::ostream &o, std::string class_name,
                               std::vector<op_type> attributes) {
  check_ostream(o);
  o << "%" << class_name << " = type {\n\t";
  for (unsigned i = 0; i < attributes.size(); ++i)
    o << attributes[i].get_name() +
             (i + 1 < attributes.size() ? ",\n\t" : "\n}\n\n");
}

void ValuePrinter::type_define(std::string class_name,
                               std::vector<op_type> attributes) {
  type_define(*stream, class_name, attributes);
}

void ValuePrinter::type_alias_define(std::ostream &o, std::string alias_name,
                                     op_type type) {
  check_ostream(o);
  o << "%" << alias_name << " = type " << type.get_name() << "\n";
}

void ValuePrinter::type_alias_define(std::string alias_name, op_type type) {
 ...
 * 	}
 */
void ValuePrinter::init_struct_constant(std::ostream &o, operand constant,
                                        std::vector<op_type> field_types,
                                        std::vector<const_value> init_values) {
  check_ostream(o);
  o << constant.get_name()
    << " = constant " + constant.get_typename() + " {\n\t";
  for (unsigned i = 0; i < init_values.size(); ++i) {
    o << field_types[i].get_name() + " ";
    if (init_values[i].get_type().get_id() == INT8 &&
        field_types[i].get_id() == INT8_PTR)
      embed_getelementptr(o, init_values[i].get_type(), init_values[i],
...
void ValuePrinter::begin_block(std::string label) {
  check_ostream();
  *stream << "\n" + label + ":\n";
}
 ...
 */
void ValuePrinter::bin_inst(std::ostream &o, std::string inst_name, operand op1,
                            operand op2, operand result) {
  check_ostream(o);
  o << "\t";
  if (!result.is_empty())
    o << result.get_name() + " = ";
  o << inst_name + " " + op1.get_typename() + " " + op1.get_name() + ", " +
           op2.get_name() + "\n";
}
operand ValuePrinter::bin_inst(std::string inst_name, operand op1,
                               operand op2) {
...
void ValuePrinter::add(std::ostream &o, operand op1, operand op2,
                       operand result) {
...
void ValuePrinter::sub(std::ostream &o, operand op1, operand op2,
                       operand result) {
...
void ValuePrinter::div(std::ostream &o, operand op1, operand op2,
                       operand result) {
...
void ValuePrinter::mul(std::ostream &o, operand op1, operand op2,
                       operand result) {
...
void ValuePrinter::xor_in(std::ostream &o, operand op1, operand op2,
                          operand result) {
 ...
 */
void ValuePrinter::malloc_mem(std::ostream &o, int size, operand result) {
  check_ostream(o);
...
void ValuePrinter::malloc_mem(std::ostream &o, operand size, operand result) {
  check_ostream(o);
 ...
 */
void ValuePrinter::alloca_mem(std::ostream &o, op_type type, operand result) {
  check_ostream(o);
 ...
 */
void ValuePrinter::load(std::ostream &o, op_type type, operand op,
                        operand result) {
  check_ostream(o);
  o << "\t";
  o << result.get_name() + " = ";
  o << "load " + type.get_name() + ", " + op.get_typename() + " " +
           op.get_name() + "\n";
}
 ...
 */
void ValuePrinter::store(std::ostream &o, operand op, operand result) {
  check_ostream(o);
 ...
 * Format: getelementptr type, op1_type op1_name, op2_type op2_name, op3_type
 * op3_name Note: This instruction can take a different number of operands but
 * you'll only need this one in MP2.1.
 */
void ValuePrinter::getelementptr(std::ostream &o, op_type type, operand op1,
                                 operand op2, operand op3, operand result) {
  check_ostream(o);
  o << "\t";
  if (result.get_type().get_id() != VOID)
    o << result.get_name() << " = ";
  o << "getelementptr " + type.get_name() + ", " + op1.get_typename() + " " +
           op1.get_name() + ", " + op2.get_typename() + " " + op2.get_name() +
           ", " + op3.get_typename() + " " + op3.get_name() + "\n";
}
operand ValuePrinter::getelementptr(op_type type, operand op1, operand op2,
                                    operand op3, op_type result_type) {
...
/* getelementptr instruction
 * Format: getelementptr type, op1_type op1_name, op2_type op2_name, op3_type
 * op3_name, op4_type op4_name Note: This instruction can take a different
 * number of operands but you'll only need this one in MP2.1.
 */
void ValuePrinter::getelementptr(std::ostream &o, op_type type, operand op1,
                                 operand op2, operand op3, operand op4,
                                 operand result) {
  check_ostream(o);
  o << "\t";
  if (result.get_type().get_id() != VOID)
...
void ValuePrinter::getelementptr(std::ostream &o, op_type type,
                                 std::vector<operand> op, operand result) {
  check_ostream(o);
  o << "\t";
  if (result.get_type().get_id() != VOID)
    o << result.get_name() << " = ";
...
operand ValuePrinter::getelementptr(op_type type, std::vector<operand> op,
                                    op_type result_type) {
  operand result = make_fresh_operand(result_type);
...
/* select instruction
 * Format: result = select op1_type op1_name, op2_type op2_name, op3_type
 * op3_name
 */
void ValuePrinter::select(std::ostream &o, operand op1, operand op2,
                          operand op3, operand result) {
  check_ostream(o);
  o << "\t" + result.get_name() + " = select " + op1.get_typename() + " " +
           op1.get_name() + ", " + op2.get_typename() + " " + op2.get_name() +
           ", " + op3.get_typename() + " " + op3.get_name() + "\n";
}
 ...
 */
void ValuePrinter::branch_cond(std::ostream &o, operand op, label label_true,
                               label label_false) {
  check_ostream(o);
  o << "\tbr " + op.get_typename() + " " + op.get_name() + ", label %" +
           label_true + ", label %" + label_false + "\n";
}
void ValuePrinter::branch_cond(operand op, label label_true,
                               label label_false) {
 ...
 */
void ValuePrinter::branch_uncond(std::ostream &o, label l) {
  check_ostream(o);
  o << "\tbr label %" + l + "\n";
}
void ValuePrinter::branch_uncond(label l) { branch_uncond(*stream, l); }

/* icmp instruction
 * Format: result_name = icmp icmp_val type op1_name, op2_name
 */
void ValuePrinter::icmp(std::ostream &o, icmp_val v, operand op1, operand op2,
                        operand result) {
  check_ostream(o);
 ...
 * ...)
 */
void ValuePrinter::call(std::ostream &o, std::vector<op_type> arg_types,
                        std::string fn_name, bool is_global,
                        std::vector<operand> args, operand result_op) {
  check_ostream(o);
  o << "\t";
...
operand ValuePrinter::call(std::vector<op_type> arg_types, op_type result_type,
                           std::string fn_name, bool is_global,
                           std::vector<operand> args) {
  operand result = make_fresh_operand(result_type);
 ...
 */
void ValuePrinter::ret(std::ostream &o, operand op) {
  check_ostream(o);
  o << "\tret ";
...
void ValuePrinter::ret(operand op) { ret(*stream, op); }

/* Bitcast instruction
 * Format: result = bitcast op_type op_name to new_type
 */
void ValuePrinter::bitcast(std::ostream &o, operand op, op_type new_type,
                           operand result) {
  check_ostream(o);
  o << "\t" << result.get_name() << " = bitcast " << op.get_typename() << " "
    << op.get_name() << " to " << new_type.get_name() << "\n";
}
 ...
 */
void ValuePrinter::ptrtoint(std::ostream &o, operand op, op_type new_type,
                            operand result) {
  check_ostream(o);
  o << "\t" << result.get_name() << " = ptrtoint " << op.get_typename() << " "
    << op.get_name() << " to " << new_type.get_name() << "\n";
}
...
operand ValuePrinter::call_variadic(std::vector<op_type> arg_types,
                                    op_type result_type, std::string fn_name,
                                    bool is_global, std::vector<operand> args) {
  operand result_op = make_fresh_operand(result_type);
  auto &o = *stream;
  o << "\t";
  if (result_op.get_type().get_id() != VOID)
    o << result_op.get_name() << " = ";
