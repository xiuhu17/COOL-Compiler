...
PARSER1="/home/zhihaow6/Desktop/426/public/mp1_handout/src/parser"
LEXER2="/home/zhihaow6/Desktop/426/public/mp1_handout/reference-binaries/lexer"
PARSER2="/home/zhihaow6/Desktop/426/public/mp1_handout/reference-binaries/parser"
...
for i in $(ls "$TEST_DIR"); do
    # 执行 lexer 和 parser 的组合并将输出保存到临时文件
    $LEXER1 "$TEST_DIR/$i" | $PARSER1 > output1.txt
    $LEXER2 "$TEST_DIR/$i" | $PARSER2 > output2.txt

    # 比较两个输出
    diff -q output1.txt output2.txt > /dev/null
    if [ $? -eq 0 ]; then
        echo "pass $i[$((PASSED_TESTS+1))/$NUM_TESTS]"
        PASSED_TESTS=$((PASSED_TESTS+1))
    else
        echo "fail $i[$((PASSED_TESTS+1))/$NUM_TESTS]"
    fi
done
