...
op_type::op_type(std::string n, int ptr_level) {
  name = "%" + n;
  id = OBJ;
...
op_type op_type::get_ptr_type() {
  ...
  if (ptr_id == OBJ_PTR || ptr_id == OBJ_PPTR) {
    op_type new_type(std::string(name.c_str() + 1), 1);
...
op_type op_type::get_deref_type() {
  ...
  if (deref_id == OBJ || deref_id == OBJ_PTR) {
    ...
    std::string new_name = name.substr(1, name.length() - 2);
...
static std::string arrayTypeName(int size, const std::string &type_name) {
  return "[" + std::to_string(size) + " x " + type_name + "]";
}
...
op_func_type::op_func_type(op_type res_type, std::vector<op_type> arg_types)
    : op_type(EMPTY), res(res_type), args(arg_types) {
  name = std::string(res_type.get_name() + " (");
  if (arg_types.size() == 0)
    name.append(") *");
  else {
    unsigned i;
...
op_func_ptr_type::op_func_ptr_type(op_type res_type,
                                   std::vector<op_type> arg_types)
    : op_type(EMPTY), res(res_type), args(arg_types) {
  op_func_type func_type(res_type, arg_types);
  name = std::string(func_type.get_name() + "*");
}
